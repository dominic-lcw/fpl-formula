...
- **Match bonus:** each completed fixture awards 3 points to the player with the highest match claim. Individual form uses the average of those awards per appearance in the rolling window (not the window total), so repeated wins help without stacking 15 raw points over five Gameweeks. The claim mixes xG + xA, threat + creativity, and defensive actions. It is separate from total FPL points and from the official FPL bonus.
