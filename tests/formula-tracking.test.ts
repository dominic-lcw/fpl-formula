...
async function seedBacktestFixtures() {
...
describe("formula backtest query", () => {
  it("aggregates historic team ids without mixing non-aggregated columns", async () => {
    ...
    const rows = await query<{
      ...
      round_points: number;
    }>(buildFormulaBacktestQuery(TRACKER_PRESET_STRATEGIES[0]!));
  ...
  it("backtests every preset in one query", async () => {
    ...
    const rows = await query<{ strategy_id: string; target_gw: number }>(
      buildMultiFormulaBacktestQuery(TRACKER_PRESET_STRATEGIES),
    );
