...
import {
  buildFormulaBacktestQuery,
  buildMultiFormulaBacktestQuery,
  TRACKER_PRESET_STRATEGIES,
} from "../src/lib/formula-tracking-data";
...
describe("formula backtest query", () => {
  it("aggregates historic team ids without mixing non-aggregated columns", async () => {
    ...
    const reader = await connection.runAndReadAll(buildFormulaBacktestQuery(TRACKER_PRESET_STRATEGIES[0]!));
  ...
  it("backtests every preset in one query", async () => {
    const connection = await createHydrationConnection();
    await connection.run(`
      INSERT INTO teams (season, team_id, name, short_name) VALUES
        ('2025-26', 10, 'Arsenal', 'ARS'),
    ...
    const reader = await connection.runAndReadAll(buildMultiFormulaBacktestQuery(TRACKER_PRESET_STRATEGIES));
    const rows = reader.getRowObjectsJS() as Array<{ strategy_id: string; target_gw: number | bigint }>;
    connection.closeSync();

    expect(new Set(rows.map((row) => row.strategy_id)).size).toBe(TRACKER_PRESET_STRATEGIES.length);
    expect(rows.length).toBe(TRACKER_PRESET_STRATEGIES.length * 2);
  });
});
